...
// for random
//				m_score = (int)(100 + (30.0 - (Math.random() * 60.0)));
//				node.getAttributeNode("score").setText(Integer.toString((int)m_score));
			} else {
				m_score	= 0.0;
			}
	...
	{
		int i, count, highestScore, lowestScore, delta, top, startX, startY, endX, endY;
		...
		if (child != null)
		{
			...
			while (child != null)
			{
				if (!child.isIgnored())
				{
					...
					if (m_lineSelectedNav && count == m_selectedTab)
						m_imgGraph.drawHighlightedVerticalRectangle(startX, startY, endX, endY, AlphaImage.makeARGB(255, 255,255,192), m_selectedColor);
					else
						m_imgGraph.drawHighlightedVerticalRectangle(startX, startY, endX, endY, AlphaImage.makeARGB(255, 255,255,192), AlphaImage.makeARGB(255,  64, 255,  64));
			...
			// Draw the graph normally
			if (m_lineSelectedNav && count == m_selectedTab)
				m_imgGraph.drawHighlightedVerticalRectangle(startX, startY, endX, endY, AlphaImage.makeARGB(255, 255,255,192), AlphaImage.makeARGB(255, 222, 22, 222));
			else
				m_imgGraph.drawHighlightedVerticalRectangle(startX, startY, endX, endY, AlphaImage.makeARGB(255, 255,255,192), AlphaImage.makeARGB(255,  64, 255,  64));
