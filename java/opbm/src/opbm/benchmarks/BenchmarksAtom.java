...
{
	public BenchmarksAtom(Benchmarks		parent,
						  BenchmarksParams	bp)
	{
...
// ABSTRACT
		...
		xmlRunAppendTo.appendChild(new Xml("error", thisCommand.getName(), "type", "Unrecognized keyword, expected flow, custom or abstract"));
		...
		for (i = 0; i < groups.size(); i++)
		{
			...
			if (groupItems.size() > 1)
			{
				...
				for (timing0Iterator = 0; timing0Iterator < timings0.size(); timing0Iterator++)
				{
					...
					extractTimingLineElements(line);
					status0	= m_timingName;								// the status name is used to identify the sub-entry within the command's group of entries, and
					timing	= m_timingInSeconds;								// the timing value is extracted (which sets our initial variable)
			...
			for (k = 0; k < timings.size(); k++)
			{
				...
				// For timing, sum
				timing += m_timingInSeconds;

				// For % of baseline, geometric mean
				if (ofBaseline == 0)
				{	// First time through, we set our value
					ofBaseline	= Math.pow(m_timingOfBaseline, power);
				} else {
...
// REMEMBER we could add additional information here
			...
			Utils.writeTerminatedLinesToFile(fileName, timings);
		}
	}
	 ...
	 * @param bp BenchmarksParams holds where to store the results based on
		...
		while (child != null)
		{
			...
			{	// See if it's an executable
				...
				{	// Grab the relevant outputs
					...
					atom = m_parent.getNextResultAtomForNextAppend(bp);
					...
					while (output != null)
					{
						...
						{	// We extract all timing lines as worklets
							extractTimingLineElements(line.substring(7));
							worklet = new Xml("worklet");
							worklet.appendAttribute("name", m_timingName);
							worklet.appendAttribute("shortname", Utils.getShortName(m_timingName, 7));
							worklet.appendAttribute("timing", Double.toString(m_timingInSeconds));
							worklet.appendAttribute("score", Double.toString(m_timingOfBaseline));
	...
	private	int					m_firstComma;
	private	int					m_secondComma;
	private	String				m_timingName;
	private	double				m_timingInSeconds;
	private	double				m_timingOfBaseline;
}
